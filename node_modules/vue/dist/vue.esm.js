...
console.groupColor= function(color = 'red', str) {
  console.group(`%c${str}`, `color:#fff;background:${color};font-size:14px`)
}
console.groupEndColor = function(color = 'red', str) {
  console.groupEnd()
...
Dep.prototype.notify = function notify () {
  ...
  for (var i = 0, l = subs.length; i < l; i++) {
    console.log(`update! depId: ${this.id}`)
    subs[i].update();
  }
};
...
function defineReactive (obj, key, val, customSetter, shallow) {
  ...
  Object.defineProperty(obj, key, {
    ...
    get: function reactiveGetter () {
      ...
      console.log(`触发了【${key}】的getter`)

      if (Dep.target) {
        const ex = Dep.target.expression
        dep.depend(); // data值的改变，通过此处来收集依赖，但是无法收集data值的增删。e.g.: this.array = 3
        if (childOb) {
          console.log(`child defineReactive, `, childOb)
          childOb.dep.depend(); // 通过此处收集data值的增删改。e.g.: this.array.push(1)
          if (Array.isArray(value)) {
            dependArray(value);
          }
        }
...
if (process.env.NODE_ENV !== 'production') {
  strats.el = strats.propsData = function (parent, child, vm, key) {
    !vm && warn("option \"" + key + "\" can only be used during instance " + 'creation with the `new` keyword.');
...
function mergeOptions (parent, child, vm) {
  console.log('原始options：', parent)
  console.log('目标options：', child)
  ...
  // 这里优化了一下哈哈
  console.time();
  const obj = Object.assign({}, parent, child)
  for (key in obj) {
    mergeField(key);
  }
  console.timeEnd();
...
function validateProp (key, propOptions, propsData, vm) {
  console.log(`validateProp start`)
  ...
  // boolean casting
  // 针对props type为boolean的情况
  var booleanIndex = getTypeIndex(Boolean, prop.type);
  if (booleanIndex > -1) {
    if (absent && !hasOwn(prop, 'default')) {
      value = false;
    }
    // hyphenate 驼峰转中划线 e.g. newVal -> new-val
  ...
  console.log(`validateProp finish`)
  return value
}
 ...
 */
function resolveSlots (children, context) { // children是父组件的传入到子组件内容，context是父组件的实例vm
  var slots = {};
  if (!children) {
    return slots
  }
  ...
  for (var i = 0, l = children.length; i < l; i++) {
    ...
    // 有名字的slot e.g. <slot name="slotA"></slot>
...
function initLifecycle (vm) {
  ...
  // non-abstract：非抽象组件，keep-alive属于抽象组件
...
var Watcher = function Watcher (vm, expOrFn, cb, options, isRenderWatcher) {
  ...
  // 重要，如果不是计算属性的话，马上执行watcher的getter方法。如果是渲染watcher的话，就是updateComopnent =》 vm._update(vm._render(), hydrating);
...
Watcher.prototype.addDep = function addDep (dep) {
  ...
  if (!this.newDepIds.has(id)) {
    ...
    if (!this.depIds.has(id)) {
      ...
      console.log(`dep: has dep id: ${id}`)
    }
  }
  else {
    console.log(`newDep: has dep id: ${id}`)
  }
};
...
function stateMixin (Vue) {
  ...
  // 将$data、$props定义到Vue原型上
  Object.defineProperty(Vue.prototype, '$data', dataDef);
  Object.defineProperty(Vue.prototype, '$props', propsDef);

  // 定义vm.$set、vm.$delete方法
...
function Vue (options) {
  // 未使用new关键字
  (process.env.NODE_ENV !== 'production' && !(this instanceof Vue)) && warn('Vue is a constructor and should be called with the `new` keyword')
...
var KeepAlive = {
  ...
  render: function () {
    ...
    var vnode = getFirstComponentChild(slot); // keep-alive只处理第一个子元素，所以一般和它搭配使用的有component动态组件或者router-view
...
function createPatchFunction ({modules, nodeOps}) {
  var i, j;
  var cbs = {};
    ...
    if (isUndef(oldVnode)) {
      ...
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        ...
        if (isRealElement) {
          ...
          //  如果是个根组件，这个oldVnode就是真实DOM<div id="app"></div>
          oldVnode = emptyNodeAt(oldVnode);
          // 将真实DOM变为Vnode，oldVnode.elm为真实DOM
        }

        // 如果是个根组件，这个就是真实DOM<div id="app"></div>
        var oldElm = oldVnode.elm;
        // 如果是个根组件，这个就是真实DOM的父节点，body
...
console.log('createPatchFunction传入的modules为：', Object.assign({}, modules))
...
function parseHTML (html, options) {
  ...
  function parseStartTag () {
    ...
    if (start) {
      ...
      if (end) {
        // 针对单闭合标签的斜线 e.g.: <input/>
        match.unarySlash = end[1]; // unarySlash = '/'
